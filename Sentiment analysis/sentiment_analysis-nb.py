...
for i in xrange( 0,len(twee)):
    senti.append(NBClassifier.classify(extract_features(getFeatureVector(process[i]))))

output = pd.DataFrame( data={"sentiment":senti, "tweet":twee} )

    # Use pandas to write the comma-separated output file
output.to_csv(os.path.join(os.path.dirname(__file__), 'data', 'C:\Users\Shaik Riyaz\Desktop\code (2)\\tweetanalysis-NaiveBayesClassifier.csv'), index=False, quoting=3)
